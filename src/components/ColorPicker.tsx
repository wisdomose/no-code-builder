...
export function ColorPicker({ color, onChange }: ColorPickerProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5 min-w-0 group relative">
      ...
      <div className="flex items-center gap-2 bg-background/50 border border-border/50 rounded-lg p-1.5 focus-within:border-primary transition-all hover:bg-background/80">
        <Popover.Root>
          <Popover.Trigger asChild>
            <button
              className="w-8 h-8 rounded-md border border-border/50 shadow-sm transition-transform hover:scale-105 active:scale-95 outline-none focus-visible:ring-2 focus-visible:ring-primary shrink-0"
        ...
        <span className="text-[11px] font-mono font-bold text-text-main truncate uppercase flex-1 px-1">
          {color}
        </span>
      </div>
    </div>
